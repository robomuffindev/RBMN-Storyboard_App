...
- **Klein "base from references" wave (1.114.0-1.125.0)** — the VNCCS Klein Hybrid clone base preview was reworked to build the character's body straight from the uploaded **reference photos** instead of a generic pose mannequin: a whole-person **ReferenceLatentPlus** channel locks build/shoulders/chest/waist/hips from up to four body/full references, posed neutrally from an empty latent (no mannequin to override the shape), with a face crop + worker RMBG. On top of that — a tunable **Strip release** that frees the final render steps to strip leftover clothing to a clean underwear base (trading likeness ↔ cleanliness), a **FaceDetailer face-refine on the base** (the same ultralytics pass the poses get — base-local On/Off + denoise/steps, with ⚙ globals), an optional **SAM3 article cleanup** that segments leftover jewelry/clothing **by name** and inpaints just those regions to skin so likeness elsewhere is untouched (the article list auto-fills from the reference analysis), and a post-hoc **photoreal Switch Style** that stacks the **anime2real-semi** LoRA on the already-rendered base and saves the result as a new active base (with a "Use realism LoRA" toggle). Realism was deliberately kept **out** of generation to keep the base render predictable. See `docs/KLEIN_MODE_PLAN.md`. **UNTESTED on a live worker.**
...
| **comfyui-ReferenceLatentPlus** (+ MediaPipe) | Masked whole-person reference channel for the Klein "base from references" build (auto-detected; MediaPipe provides the segmentation masks) | [github.com/shootthesound/comfyui-ReferenceLatentPlus](https://github.com/shootthesound/comfyui-ReferenceLatentPlus) |
| **ComfyUI-Easy-Sam3** (+ SAM3 model) | Optional text-prompted article cleanup on the Klein base (`easy sam3ImageSegmentation`; needs a SAM3 model, e.g. `sam3-fp16.safetensors`) | [github.com/yolain/ComfyUI-Easy-Sam3](https://github.com/yolain/ComfyUI-Easy-Sam3) |
