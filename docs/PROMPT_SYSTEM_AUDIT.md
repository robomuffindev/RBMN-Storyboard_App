# Prompt Enhancement & System-Prompt Audit

_Auto-generated map of the prompt system — v1.18.0, 2026-06-28. Use this to review how each scene's prompts are derived and to find issues._

This document is generated from the live code (`backend/services/llm/prompt_enhancer.py`, `backend/api/generation.py`, `backend/services/jobs/dispatcher.py`, `frontend/src/components/SceneEditor/SceneEditor.tsx`). The system prompts in §8 are extracted **verbatim**.

---

## 1. Executive summary

Every image/video prompt is produced by an **LLM enhancement step**: a *user prompt* (or empty) plus a *context string* assembled from the project + scene, run through a **model- and role-specific system prompt**. The LLM returns a single clean paragraph. That paragraph is stored on the scene, and at **dispatch time** the renderer appends mechanical suffixes (SFW, image-direction tag, colour palette, anti-text) to produce the **exact string sent to ComfyUI** (`submitted_*_prompt`).

Two trigger paths feed the same `PromptEnhancer.enhance()` core:

- **Manual Enhance** — the per-scene **Enhance** button (Image/Video tabs). Context built on the **frontend** (`buildEnhanceContext`).
- **Auto-Gen** — the sequential/batch pipeline. Context built on the **backend** (`_build_auto_enhance_context` for images, `_build_video_enhance_context` for video).
...
```mermaid
flowchart TD
    A[Trigger] -->|Manual Enhance button| B1[buildEnhanceContext - frontend]
    A -->|Auto-Gen sequential/batch| B2[_build_auto_enhance_context / _build_video_enhance_context - backend]
    B1 --> C[Context string]
    B2 --> C
    C --> D[PromptEnhancer.enhance]
    UP[User prompt or empty] --> D
    D --> E{Resolve prompt_type}
    E -->|two_pass base/composite| F1[two_pass_base / two_pass_composite]
    E -->|is_video| F2[video]
    E -->|frame_type=last| F3[image_last_frame]
    E -->|else| F4[image]
    F1 --> G[get_system_prompt: override -> model built-in -> generic]
    F2 --> G
    F3 --> G
    F4 --> G
    G --> H[LLM call: system + 'Original prompt' + 'Context']
    H --> I[_collapse_to_single_paragraph + _clean]
    I --> J[Stored on scene: prompt / last_frame_prompt / video_prompt]
    J --> K[DISPATCH: append SFW + image-direction + colour + anti-text suffixes]
    K --> L[submitted_*_prompt -> ComfyUI workflow node]
```

> Ideogram JSON mode is a parallel branch: the prose is converted to a structured caption via `JSON_PROMPT_SYSTEM_PROMPT` (see §7).

---

## 3. Triggers (where enhancement starts)

| Path | Where context is built | Function | Notes |
|---|---|---|---|
| Manual Enhance (Image tab, First frame) | Frontend | `buildEnhanceContext` + base | `frame_type='first'` |
| Manual Enhance (Image tab, Last frame) | Frontend | `buildEnhanceContext` + base | `frame_type='last'` |
| Manual Enhance (Video tab) | Frontend | `buildEnhanceContext` + video base | `is_video=true` |
| Auto-Gen image (first/last) | Backend | `_build_auto_enhance_context(...)` | per scene, per frame |
| Auto-Gen video | Backend | `_build_video_enhance_context(...)` | per scene |
| JSON Prompt 'Generate with AI' | Backend | `build_json_prompt` endpoint | structured caption |
| Auto-Gen Story Flow (pre-step) | Backend | flow LLM (separate) | writes `scene.parameters.flow_idea` used by the context above |

---

## 4. System-prompt selection

`enhance()` derives a **prompt_type** then looks it up for the **generation model**. Fallback chain: **user override** → **model built-in** → **generic default**.

```mermaid
flowchart TD
    S[enhance called] --> T{two_pass_phase?}
    T -->|base| TB[prompt_type = two_pass_base]
    T -->|composite| TC[prompt_type = two_pass_composite]
    T -->|no| U{is_video?}
    U -->|yes| UV[prompt_type = video]
    U -->|no| V{frame_type == last?}
    V -->|yes| VL[prompt_type = image_last_frame]
    V -->|no| VI[prompt_type = image]
    TB --> W[get_system_prompt model_key, prompt_type]
    TC --> W
    UV --> W
    VL --> W
    VI --> W
    W --> X{user override enabled?}
    X -->|yes| XO[use override text]
    X -->|no| Y{model has built-in for prompt_type?}
    Y -->|yes| YB[use model built-in]
    Y -->|no| YG[use generic default]
```

### Selection matrix (model × prompt_type → system prompt)

| Generation model (`gen_model_name`) | image | image_last_frame | two_pass_base | two_pass_composite | video |
|---|---|---|---|---|---|
| `flux2_klein_dev_9b` (default) | IMAGE | LAST_FRAME | TWO_PASS_BASE | TWO_PASS_COMPOSITE | — |
| `flux1_dev` | IMAGE | LAST_FRAME | (generic) | (generic) | — |
| `z_image` | Z_IMAGE | LAST_FRAME | TWO_PASS_BASE | (generic) | — |
| `krea2` | KREA2 | LAST_FRAME | TWO_PASS_BASE | (generic) | — |
| `qwen_edit` | QWEN_EDIT | LAST_FRAME | (generic) | (generic) | — |
| `ltx_2.3` / `wan_2.2` (video) | — | — | — | — | VIDEO |
| narration mode (image/video) | NARRATION_IMAGE | LAST_FRAME | — | — | NARRATION_VIDEO |

> **Override**: Settings → per-model system-prompt override replaces the built-in when enabled. Per-model **guidance** text (if set) is appended to the system prompt as `ADDITIONAL PROMPT RULES`.

> **Note / potential issue**: narration prompts are selected in the API layer (`enhance_prompt` swaps in `NARRATION_*` when `project.mode` is narration and the model isn't krea2/z_image), so a narration project using Krea2/Z-Image as first-pass gets the **Krea2/Z-Image** image prompt, not the narration one. Worth reviewing if narration scenes look off on those models.

---

## 5. Context data-source hierarchy (what feeds the LLM)

The **context string** is the pipe-joined list below, in this order. Earlier = appears earlier in context; **priority** notes which items are framed as overriding others.

### 5a. Image context — `_build_auto_enhance_context` (auto) — order

| # | Source | Scene/Project field | Fires when | Priority framing |
|---|---|---|---|---|
| 1 | **User LLM instruction** | `parameters.llm_instruction_image` | set | **HIGHEST — overrides all** |
| 2 | Model info | `model_type` | always | — |
| 3 | Scene timing + frame | `start_time/end_time`, frame | always | — |
| 4 | **Video starting-frame rule** | project.mode animated + frame=first | animated scenes | strong (LTX I2V) |
| 5 | Concept | `settings.concept_text` | set | — |
| 6 | Visual style | `settings.style_text` | set | — |
| 7 | **Global project context** | `settings.global_context_*` | toggled on | **MANDATORY** wrapper |
| 8 | Image direction / art style | `settings.image_direction` | != none | — |
| 9 | **Colour palette override** | `parameters.color_override` / global | != full_color | **HIGHEST — absolute** |
| 10 | Characters (by image position) | `settings.characters[:2]` (first frame) | present | references attached |
| 10L | **Last-frame cast block** | LF selected/flow chars + FF prompt | frame=last | in/out/enters |
| 11 | **Scene lyrics** | `scene_lyrics` | set | **PRIMARY creative source** |
| 12 | Scene storyboard / flow | `parameters.flow_idea` | set + `use_story_flow` | composition/camera |
| 13 | Camera action | `parameters.camera_action` | != none | — |
| 14 | Previous-scene continuity | prev `last_frame_prompt` | `use_prev_scene_last_frame` | style only |
| 15 | **Vision ref descriptions** | Ollama vision on attached refs | vision enabled + refs | precedes text assumptions |
| 16 | **Ideogram ref layouts** | `asset.meta.ideogram_caption` | ref made via Ideogram | positions + composition |

### 5b. Video context — `_build_video_enhance_context` (auto)

Same project-level items, plus: video model + mode, scene duration, camera action, keyframe awareness (first/last frame images already define look → describe **motion** between them), prev-scene continuity, vision ref descriptions, Ideogram ref layouts.

### 5c. Manual context — `buildEnhanceContext` (frontend)

A lighter set (the heavy per-scene resolution is server-side at generate time): concept → visual style → image direction → characters → **scene lyrics (PRIMARY)** → scene storyboard. The caller prepends a **base** with model + timing + frame + reference descriptions, the **last-frame** continuity block, and the **video-starting-frame** rule (for animated first frames). Vision + Ideogram-layout blocks are added server-side in `enhance_prompt`.
...
flowchart LR
    subgraph Project
      C1[concept] --> CTX
      C2[style] --> CTX
      C3[image direction] --> CTX
      C4[global context] --> CTX
      C5[characters] --> CTX
    end
    subgraph Scene
      S1[lyrics PRIMARY] --> CTX
      S2[flow_idea storyboard] --> CTX
      S3[color override] --> CTX
      S4[camera action] --> CTX
      S5[llm_instruction HIGHEST] --> CTX
      S6[prev-scene continuity] --> CTX
    end
    subgraph References
      R1[vision descriptions] --> CTX
      R2[ideogram ref layouts] --> CTX
    end
    CTX[Context string] --> ENH[enhance + system prompt]
    ENH --> OUT[Enhanced paragraph]
```

---

## 6. The end result + dispatch suffixes

The LLM output is cleaned (`_collapse_to_single_paragraph`, `_clean_enhanced_prompt` — strips labels, quotes, booster spam, weight syntax) and stored as `scene.prompt` (first frame), `parameters.last_frame_prompt`, or `parameters.video_prompt`.

At **dispatch**, the renderer appends mechanical suffixes to form the exact ComfyUI string (`submitted_*_prompt`, visible read-only on the Prompt tab):

| Order | Suffix | When |
|---|---|---|
| 1 | `, SFW, safe for work, fully clothed, no nudity, no explicit content, no NSFW` | SFW setting on |
| 2 | image-direction **style tag** (e.g. `, anime style`) | image_direction set |
| 3 | colour suffix `, <palette>` (+ enforcement clause) | colour override set |
| 4 | `, no text, no subtitles, no captions, no words, no letters, no watermarks` | image first-pass models |

> Klein has **no negative-prompt node** — any scene/global negative prompt is dropped at dispatch (logged).

---

## 7. Special paths

**Two-pass (character compositing, Klein).** Pass 1 = `two_pass_base` (Z-Image/Krea2, no refs, balanced exposure — builds the empty scene). Pass 2 = `two_pass_composite` (Klein edit instruction — inserts the character refs into the Pass-1 image; SHORT 'Image N' edit directions, no character names).

**Ideogram JSON mode (Krea2).** Prose → `JSON_PROMPT_SYSTEM_PROMPT` → structured caption (global summary + style + background + spatially-placed elements with x/y/w/h boxes + palettes). Built by the Enhance button (when mode on) or lazily at dispatch; editable in the JSON Prompt editor; stored on `scene.parameters.json_prompt` and on the generated asset's `meta.ideogram_caption`.

---

## 8. The system prompts (verbatim)

_Extracted directly from `backend/services/llm/prompt_enhancer.py`._

### IMAGE_SYSTEM_PROMPT — default first-frame image (FLUX.2 Klein)

```text
You are an expert at writing prompts for FLUX.2 Klein 9B (also used for FLUX.1) — a natural-language image model
...
### Z_IMAGE_SYSTEM_PROMPT — Z-Image Turbo first-pass
...
### KREA2_IMAGE_SYSTEM_PROMPT — Krea 2 Turbo first-pass
...
### QWEN_EDIT_SYSTEM_PROMPT — Qwen-Image-Edit

```text
You are an expert at writing prompts for Qwen-Image-Edit (Alibaba), an instruction-driven
...
### LAST_FRAME_IMAGE_SYSTEM_PROMPT — last frame (FF/LF mode)

```text
You are an expert at writing prompts for FLUX.2 Klein 9B, creating the LAST FRAME of a video scene.
...
### TWO_PASS_BASE_SYSTEM_PROMPT — two-pass Pass 1 (empty scene)

```text
You are an expert at writing prompts for Z-Image Turbo, an AI image generation model
...
### TWO_PASS_COMPOSITE_SYSTEM_PROMPT — two-pass Pass 2 (Klein edit)

```text
You write a SHORT, literal EDIT INSTRUCTION for FLUX.2 Klein 9B — an image EDIT model that ALREADY SEES the
...
### JSON_PROMPT_SYSTEM_PROMPT — Ideogram structured caption
...
### VIDEO_SYSTEM_PROMPT — LTX 2.3 video (LTXDirector)

```text
You are an expert at writing prompts for LTX Video 2.3 with the LTXDirector node, an advanced AI video generation system.
...
### NARRATION_IMAGE_SYSTEM_PROMPT — narration mode image

```text
You are an expert at writing prompts for FLUX.2 Klein 9B, a reference-image-conditioned AI image generation model.
...
### NARRATION_VIDEO_SYSTEM_PROMPT — narration mode video
...
## 9. Audit checklist — where issues tend to hide

- **Context vs system-prompt conflict** — e.g. lyrics say 'actions MUST appear' while the video-starting-frame rule says 'opening moment only'. Check ordering/priority in §5a.
- **Character cap of 2** — image context uses `characters[:2]`; scenes with 3+ characters silently drop the rest.
- **Narration model mismatch** (§4 note) — narration project on Krea2/Z-Image first-pass uses the non-narration prompt.
- **Lyrics 'MUST appear'** — strong wording can over-stuff busy scenes; the de-fluffed model prompts soften this but the context line is still forceful.
- **Flow idea quality** — the storyboard (`flow_idea`) drives composition; a weak flow → weak scene. It's generated in a separate pre-step.
- **Suffix collisions** — colour override + image-direction tag + SFW can pile competing style cues onto the tail at dispatch (§6).
- **Vision/ideogram only when enabled/available** — ref descriptions and layouts are skipped silently if vision is off or the ref wasn't Ideogram-made.
- **Manual vs auto drift** — the frontend `buildEnhanceContext` is lighter than the backend builder; the same scene can enhance differently via the button vs auto-gen.

To inspect any single scene end-to-end, use the **Prompt tab → Download Prompts JSON** (every prompt + the exact `submitted_to_comfy` strings + models + refs + Ideogram caption).


---

## 10. Fixes applied (v1.19.0) — from external review

The valid items from §9 + the Gemini/ChatGPT review were implemented:

| Item | Status |
|---|---|
| LTX multi-segment collapse | **Not a bug** — `_clean_prompt_preserve_segments` (renamed) already preserves segments. Verified. |
| Aspect-ratio blindness | Fixed — `TARGET CANVAS` injected (auto image+video, manual). |
| First-frame vs lyrics conflict | Fixed — resolution clause (depict the charged moment just before). |
| Narration routing on Krea2/Z-Image | Fixed — model-agnostic `NARRATION MODE` directive. |
| Competing priority labels | Fixed — explicit `PRIORITY ORDER` stack. |
| Palette over-strict on photoreal skin | Fixed — strict (monochrome) vs colour-grade auto-detection. |
| Two-pass relight destroying faces | Fixed — `PRESERVE THE FACE` clause in Pass 2. |
| Character cap silent drop | Fixed — context note + log + Prompt-tab warning. |
| Dispatch suffix transparency | Fixed — `dispatch_mutations` in the prompt export. |

**Deferred (larger / higher-risk, scoped separately):** full backend context centralization (manual vs auto parity was improved instead), removing the anti-text/SFW dispatch suffixes (surfaced via `dispatch_mutations` rather than removed — render-behaviour change), the full pre-dispatch validator suite, and the Scene Intent Object two-stage compiler.
