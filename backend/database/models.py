...
class Scene(SQLModel, table=True):
    ...
    # Human-readable shortcode — "a3f9-sce-005"
...
class Asset(SQLModel, table=True):
    ...
    # Human-readable shortcode — "a3f9-img-0047"
    short_code: Optional[str] = Field(default=None, index=True, unique=True)
...
class AppSettings(SQLModel, table=True):
    ...
    chapter_auto_split_threshold: int = Field(default=25)
    chapter_max_depth: int = Field(default=2)


class Chapter(SQLModel, table=True):
    """Narrative chapter / sub-chapter.

    Chapters are an umbrella over a contiguous range of scenes.  Top-level
    chapters have ``parent_chapter_id=None``; sub-chapters reference
    their parent.  Depth is capped at 2 (so three levels: 0, 1, 2).

    Chapters can be derived from Markdown-style ``# header`` markers in
    the project's script (Lyrics.initial_text) OR auto-generated by
    splitting scenes into batches of ``chapter_auto_split_threshold``.

    The ``source`` field tracks origin so script-edit reparsing won't
    overwrite a user's manual rename / color choice.
    """
    ...
    parent_chapter_id: Optional[UUID] = Field(
        default=None, foreign_key="chapters.id", index=True
    )
    order_index: int = Field(default=0)
    depth: int = Field(default=0)  # 0 = top-level, 1 = sub, 2 = sub-sub
    name: str = Field(default="")
    short_code: str = Field(index=True, unique=True)
    color: str = Field(default="#7c3aed")
    auto_generated: bool = Field(default=True)
    source: str = Field(default="auto")  # auto | script_header | manual
    script_offset_start: Optional[int] = Field(default=None)
    script_offset_end: Optional[int] = Field(default=None)
...
class ShortcodeCounter(SQLModel, table=True):
    """Atomic per-(project, entity-type) sequence allocator.

    One row per (project_id, type_code) tracks the NEXT sequence number
    to hand out.  Allocation is done in a single UPDATE+SELECT to avoid
    races.  Type codes are 2-3 lowercase letters: ``img``, ``vid``,
    ``aud``, ``ref``, ``chr``, ``clo``, ``itm``, ``plc``, ``mus``,
    ``nar``, ``sce``, ``ch``, ``bt``.
    """

    __tablename__ = "shortcode_counters"

    id: UUID = Field(default_factory=uuid4, primary_key=True)
    project_id: UUID = Field(foreign_key="projects.id", index=True)
    type_code: str = Field(index=True)
    next_seq: int = Field(default=1)
    # Server-side default so raw-SQL INSERTs that omit this column work.
    updated_at: Optional[datetime] = Field(
        default_factory=datetime.utcnow,
        sa_column_kwargs={"server_default": "CURRENT_TIMESTAMP", "nullable": True},
    )
