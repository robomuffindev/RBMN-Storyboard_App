...
# v1.199.144: the MANNEQUIN's rest bone directions, in VIEWER axes (Y up), measured
# from pose_render._apply_pose(base, {}, ...) with return_joints.  EVERY authored pose --
# the VNCCS library and ours -- is written against THIS stance, then applied to a rig whose
# own rest is different (T-posed since the v120 canonicalization).  Measured mismatch on
# Duke, degrees from vertical:  upper arm 42.2 vs 85.4 (+43) · clavicle 77.9 vs 84.3 (+6)
# · thigh 6.4 vs 15.9 (+10) · foot 61.9 vs 47.0 (-15) · calf 8.2 vs 7.5 (ok).
# Those three leftovers are exactly what Lorenzo reported after v143 fixed the arms alone:
# "shoulders a bit over exaggerated" and "standing on his heels instead of flat".  So align
# EVERY mapped bone, not one chain.
MANNEQUIN_REST_DIR = {
    "spine_01": (0.00000, 0.98440, 0.17595),
    "spine_02": (-0.00000, 0.99793, 0.06429),
    "spine_03": (-0.00000, 0.99889, 0.04719),
    "neck_01": (0.00000, 0.93233, 0.36161),
    "clavicle_l": (0.97442, -0.21032, -0.07923),
    "upperarm_l": (0.67131, -0.74104, -0.01397),
    "lowerarm_l": (0.50532, -0.49693, 0.70549),
    "clavicle_r": (-0.97442, -0.21032, -0.07923),
    "upperarm_r": (-0.67131, -0.74104, -0.01397),
    "lowerarm_r": (-0.50532, -0.49693, 0.70549),
    "thigh_l": (0.06651, -0.99386, 0.08847),
    "calf_l": (0.10384, -0.98968, -0.09878),
    "foot_l": (0.03377, -0.47095, 0.88151),
    "thigh_r": (-0.06651, -0.99386, 0.08847),
    "calf_r": (-0.10384, -0.98968, -0.09878),
    "foot_r": (-0.03377, -0.47095, 0.88151),
}
...
def main() -> int:
    ...
    # v1.199.115: NORMAL MODE. An arm pressed against the torso is nearly
    # invisible in a depth map (near-zero tonal separation even after the 16-bit
    # percentile re-stretch) but produces a hard surface-ORIENTATION
    # discontinuity in a normal map -- exactly the remaining tucked-arm defect
    # of the depth path. Rendered via Workbench matcap below; consumed by the
    # RefControl NORMAL LoRA (same family/recipe as the depth one).
    want_normal = render_mode == "normal"

    # camera + render settings (Workbench clay)
    sc = bpy.context.scene
    sc.render.engine = "BLENDER_WORKBENCH"
    if want_normal:
        # v1.199.115: the built-in "check_normal+y" MATCAP encodes the VIEW-SPACE
        # normal as normal-map RGB (R=+X right, G=+Y up, B=+Z toward camera) --
        # the same convention the DSINE preprocessor emits, which is what the
        # RefControl NORMAL LoRA was trained on. View transform MUST be Standard
        # or the colours stop being an encoding. Cavity would shade over the
        # encoding, so it is off here.
        try:
            sc.display.shading.light = "MATCAP"
            _mats = [getattr(_s, "name", "") for _s in
                     bpy.context.preferences.studio_lights
                     if getattr(_s, "type", "") == "MATCAP"]
            _pick = next((_n for _n in _mats if "check_normal" in _n.lower()),
                         next((_n for _n in _mats if "normal" in _n.lower()), None))
            if not _pick:
                print(f"clay_driver: no normal matcap in this Blender (matcaps={_mats})",
                      file=sys.stderr, flush=True)
                return 5
            sc.display.shading.studio_light = _pick
            print(f"CLAY_LOG normal matcap: {_pick}", flush=True)
        except Exception as _nme:  # noqa: BLE001
            print(f"clay_driver: normal matcap setup failed ({_nme})",
                  file=sys.stderr, flush=True)
            return 5
    ...
    # v1.199.143: REST-POSE OFFSET.  Every authored pose -- the VNCCS library and
    # ours -- was written against the MANNEQUIN, whose rest stance holds the upper
    # arms 42.2 deg from vertical (measured: pose_render._apply_pose joints, bones
    # {}).  Since the v120 canonicalization the rigged MESH rests in a T-pose --
    # measured on Duke's own rigged.fbx in Blender: LeftArm 85.3 deg, RightArm
    # 85.6 deg from vertical.  Same numbers, two different starting stances, so in
    # clay mode every arm sits ~43 deg too HIGH.  That is the "scarecrow arms" in
    # every pose render, and it is not the pose data: it is the frame the pose is
    # applied in.  Measured end to end -- authoring `upperarm z = -25` produced a
    # clay control whose arm read 62.6 deg from vertical (85 - 25 = 60, not 42 - 25
    # = 17).  So adduct by the difference first; a pose value of 0 then means the
    # same stance in both systems.  Computed per rig, so an old arms-down mesh
    # (rest ~15 deg) gets a small NEGATIVE offset instead of a big positive one.
    _MANNEQUIN_REST_DEG = 42.2
    _rest_off = 0.0
    try:
        import math as _m3
        _angs = []
        for _mh in ("upperarm_l", "upperarm_r"):
            _mx = mapped.get(_mh)
            _bd = arm.data.bones.get(_mx) if _mx else None
            if _bd is None:
                continue
            _h = arm.matrix_world @ _bd.head_local
            _t = arm.matrix_world @ _bd.tail_local
            _d = (_t - _h)
            if _d.length < 1e-9:
                continue
            _d = _d.normalized()
            _angs.append(_m3.degrees(_m3.atan2(_m3.sqrt(_d.x * _d.x + _d.y * _d.y), -_d.z)))
        if _angs:
            _rest_off = (sum(_angs) / len(_angs)) - _MANNEQUIN_REST_DEG
            print(f"CLAY_LOG rest offset: rig arms {sum(_angs)/len(_angs):.1f} deg from "
                  f"vertical vs mannequin {_MANNEQUIN_REST_DEG:.1f} -> adducting poses by "
                  f"{_rest_off:.1f} deg", flush=True)
    except Exception as _re:
        print(f"CLAY_LOG rest offset: skipped ({_re})", flush=True)
    ...
    def apply_pose(pose):
        ...
        # v1.199.145: BACK TO THE v143 UPPER-ARM-ONLY OFFSET.  The v144 per-bone world
        # alignment is WRONG and is disabled below -- see the note there.
        # v1.199.146: the SHOULDER gets the same single-bone treatment as the arm.
        # Measured rest, degrees from vertical: clavicle 84.3 (rig) vs 77.9 (mannequin),
        # so the shoulders ride ~6.4 deg high -- Lorenzo, twice: "the shoulders are a bit
        # over exaggerated".  ONE bone, one scalar, no chain composition (that is what
        # broke v144).  The upper arm HANGS OFF the clavicle, so it inherits this
        # rotation: its own offset is reduced by the same amount or the arm is corrected
        # twice.  Feet (rig 47.0 vs mannequin 61.9) are deliberately NOT touched here --
        # the ankle axis sign is unverified and a wrong guess is worse than 15 deg.
        _CLAV_OFF = 6.4
        if abs(_rest_off) > 0.5:
            bones_deg = dict(bones_deg)
            for _bn, _sgn in (("clavicle_l", -1.0), ("clavicle_r", 1.0)):
                _c0 = list(bones_deg.get(_bn) or [0.0, 0.0, 0.0])
                while len(_c0) < 3:
                    _c0.append(0.0)
                _c0[2] = float(_c0[2]) + _sgn * _CLAV_OFF
                bones_deg[_bn] = _c0
            for _bn, _sgn in (("upperarm_l", -1.0), ("upperarm_r", 1.0)):
                _r0 = list(bones_deg.get(_bn) or [0.0, 0.0, 0.0])
                while len(_r0) < 3:
                    _r0.append(0.0)
                _r0[2] = float(_r0[2]) + _sgn * (_rest_off - _CLAV_OFF)
                bones_deg[_bn] = _r0
        if _abduct:
            bones_deg = dict(bones_deg)
            for _bn, _sgn in (("upperarm_l", -1.0), ("upperarm_r", 1.0)):
        ...
        # v1.199.144 REST ALIGNMENT: per bone, the rotation that carries the RIG's rest
        # direction onto the MANNEQUIN's, both in armature space.  Composed BEFORE the
        # pose (Rw @ A) so a pose value of 0 renders the mannequin's stance, which is the
        # frame every authored pose assumes.  Applied even to bones the pose does not
        # touch -- otherwise an unposed shoulder or ankle keeps the rig's rest and the
        # mismatch survives.
        _align = {}
        # v1.199.145 RETRACTED: the v144 alignment below is composed per bone in WORLD
        # space and ignores the CHAIN -- aligning the spine already rotates everything
        # under it, so the child's own alignment double-applies.  Field result: mangled
        # clay, twisted head, arm stubs.  A correct version has to work in each bone's
        # PARENT-relative frame (a real retarget), not bone-by-bone in world axes.
        # Left in place, switched off, so the next attempt starts from the measurements
        # rather than from scratch.
        _ALIGN_ENABLED = False
        if _ALIGN_ENABLED:
          try:
              from mathutils import Vector as _V
              _C3 = C.to_3x3()
              for _mh, _mx in mapped.items():
                  _md = MANNEQUIN_REST_DIR.get(_mh)
                  if not _md or _mx not in rest:
                      continue
                  _rig = (rest[_mx].to_quaternion() @ _V((0.0, 1.0, 0.0))).normalized()
                  _man = (_C3 @ _V(_md)).normalized()
                  if _rig.length < 1e-6 or _man.length < 1e-6:
                      continue
                  _a = _rig.angle(_man)
                  if _a < 0.0087:                      # under 0.5 deg -- nothing to correct
                      continue
                  _align[_mh] = _rig.rotation_difference(_man).to_matrix()
              if _align and not getattr(apply_pose, "_align_logged", False):
                  apply_pose._align_logged = True
                  import math as _m4
                  _sm = ", ".join(
                      f"{k} {_m4.degrees((rest[mapped[k]].to_quaternion() @ _V((0.0,1.0,0.0))).angle(_C3 @ _V(MANNEQUIN_REST_DIR[k]))):.0f}deg"
                      for k in sorted(_align))
                  print(f"CLAY_LOG rest alignment ({len(_align)} bones): {_sm}", flush=True)
          except Exception as _ae:
              print(f"CLAY_LOG rest alignment skipped ({_ae})", flush=True)
        ...
        for mh, mix_name in mapped.items():
            ...
            if not _has_rot and abs(_extra) < 1e-4 and _A is None:
                continue
            ...
            if _A is not None:
                Rw = Rw @ _A            # rig rest -> mannequin rest, THEN the pose
            if abs(_extra) > 1e-4:
    ...
    fill_holes = bool(job.get("fill_holes", want_depth or want_normal))
