...
async def autogenerate_characters(
    ...
    # 🧬 v1.277.12 — HIS CALL: project auto character generation goes through
    # THE CHARACTER BUILDER (⚡ Autogen: real klein3 characters with a picked
    # base), not the old one-off klein_t2i render. Each character becomes a
    # library character; when its base is done, a watcher imports the image
    # back into the project (via the adopt-k3 endpoint, so DB work happens in
    # a real session — the thread only does HTTP, per the threading rules).
    from backend.api.autogen import AutogenSpec, _enqueue, _slugify
    ...
    for char in new_characters:
        desc = (char.description or char.name or "a character").strip()
        if combined_char_style:
            desc = f"{desc}. Visual style: {combined_char_style}"
        specs.append(AutogenSpec(name=char.name, description=desc[:2000],
                                 do_base=True, do_views=False))
    res = _enqueue(specs, label=f"project cast: {project.name}")
    jobs = res.get("jobs") or []
    job_ids = [j["id"] for j in jobs]

    # remember the slug each character will get (write-back is by INDEX)
    settings = dict(project.settings or {})
    chars_st = settings.get("characters") or []
    for idx, char in enumerate(new_characters):
        if idx < len(chars_st):
            chars_st[idx]["char_slug"] = _slugify(char.name)
    settings["characters"] = chars_st
    project.settings = settings
    await session.commit()

    _watch_autogen_cast(str(project_id),
                        [(idx, j["id"], _slugify(new_characters[idx].name))
                         for idx, j in enumerate(jobs)])

    logger.info(
        f"Autogenerated {len(new_characters)} characters for project {project_id} "
        f"via the character builder (autogen queue: {len(job_ids)} jobs)"
    )

    return AutogenCharactersResponse(
        characters=new_characters,
        job_ids=job_ids,
        message=(f"Generated {len(new_characters)} characters — building them "
                 f"in the character studio (⚡ Autogen). Images appear as each "
                 f"base finishes (~2 min per character)."),
    )


def _watch_autogen_cast(project_id: str, items: list) -> None:
    """Background thread: wait for each autogen base, then hand the result to
    the adopt-k3 endpoint (HTTP self-call — legal from a THREAD, never from a
    route; the v1.276.41 rule)."""
    ...
    def _get(path: str):
        with urllib.request.urlopen("http://127.0.0.1:8899" + path,
                                    timeout=30) as r:
    ...
    def _loop():
        pending = list(items)
        t0 = _time.time()
        while pending and _time.time() - t0 < 3600 * 4:
            _time.sleep(10)
            still = []
            for idx, jid, slug in pending:
                try:
                    st = _get(f"/api/autogen/jobs/{jid}?log=0")
                    stage = st.get("stage")
                    if stage == "done":
                        _post(f"/api/projects/{project_id}/concept/characters/"
                              f"{idx}/adopt-k3", {"slug": st.get("slug") or slug})
                        logger.info("project cast watcher: %s adopted (#%s)",
                                    slug, idx)
                    elif stage in ("error", "cancelled"):
                        logger.warning("project cast watcher: %s %s: %s",
                                       slug, stage, st.get("error"))
                    else:
                        still.append((idx, jid, slug))
                except Exception as e:                           # noqa: BLE001
                    logger.warning("project cast watcher poll failed: %s", e)
                    still.append((idx, jid, slug))
            pending = still

    threading.Thread(target=_loop, daemon=True,
                     name=f"cast-watch-{project_id[:8]}").start()


class AdoptK3In(BaseModel):
    slug: str


@router.post("/characters/{char_index}/adopt-k3")
async def adopt_k3_character(
    project_id: UUID,
    char_index: int,
    body: AdoptK3In,
    session: AsyncSession = Depends(get_session),
):
    """Import a finished klein3 character's active base into the project as
    this character's image (called by the cast watcher; also usable by hand)."""
    from backend.api.projects import _import_k3_base_as_asset
    project = await session.get(Project, project_id)
    if not project:
    ...
    if not (0 <= char_index < len(chars)):
        raise HTTPException(404, f"no character at index {char_index}")
    rel = await _import_k3_base_as_asset(project, body.slug.strip(), session)
    if not rel:
        raise HTTPException(400, f"{body.slug!r} has no active base to import")
    chars[char_index] = {**chars[char_index], "image_path": rel,
                         "char_slug": body.slug.strip()}
    st["characters"] = chars
    project.settings = st
    await session.commit()
    return {"image_path": rel, "char_slug": body.slug.strip()}
