const MORPH_URL = "/api/studio/vnccs/r/character_studio/morph_data.bin";
...
self.onmessage = async (event) => {
    ...
    try {
        ...
        if (type === "init") {
            const d = await loadMorphData();
            // topology only (bones + joint index arrays) for the first viewer.loadData()
            const jointsOut = {};
            for (const [name, idx] of Object.entries(d.joints || {})) jointsOut[name] = Array.from(idx);
            self.postMessage({ type: "meshinfo", clientId, bones: d.bones || [], joints: jointsOut });
            return;
        }
