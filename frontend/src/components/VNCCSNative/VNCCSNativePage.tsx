...
type Tab = 'create' | 'clothes' | 'emotions' | 'cloner' | 'poselib' | 'library';
...
function MannequinStrip({ character, hosts, sel, onSelect, hint, costume, onOpen }: {
  ...
  return (
    <div>
      ...
      {hint && <p style={{ fontSize: 11, color: '#6b7280', margin: '4px 0 0' }}>{hint}</p>}
    </div>
  );
}
...
export default function VNCCSNativePage({ variant = 'native' }: { variant?: 'native' | 'klein' }) {
  ...
  const loadHost = useCallback(async () => {
    try {
      ...
      if (ps) {
        ...
        if (Array.isArray(ps.extras)) setExtraPoses(ps.extras.filter((x) => x && x.pose));
      }
  ...
  const previewStatus = previewBusy ? (
    <div style={{ marginTop: 8, marginBottom: 4 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#9aa4b2', marginBottom: 3 }}>
  ...
  const runGenerate = async (step: VNCCSStepT, charName: string, body: Partial<api.GenerateBody>,
    ...
    if (_eng !== 'klein' || step === 'creator' || step === 'cloner') {
  ...
  const doPreview = async () => {
    ...
    setPreviewBusy(true); setErrMsg(''); setSaveMsg('');
    try {
  ...
  const cloneRefsForGen = () =>
    cloneRefs.map((r) => {
      ...
      return { ...base, role: (r.role || 'full') as api.RefRole };  // role always rides from the original
    });
  ...
  useEffect(() => {
    if (!settingsLoaded.current) return;
    if (createSub !== 'clone' || !editingCharId || !cloneName.trim()) return;
  ...
  const addLibraryPose = async (p: { id: string; name: string; category: string; repository: string; pose: Record<string, unknown> | null }) => {
    ...
    setExtraPoses((prev) => prev.some((x) => x.id === p.id) ? prev
      : [...prev, { id: p.id, name: p.name, pose: pose!,
                    thumbUrl: api.poseLibraryPreviewUrl(p.name, p.repository, p.category) }]);
  ...
  const kleinBaseControls = (nsfw: boolean, setNsfw: (b: boolean) => void,
    ...
    <div style={{ display: 'grid', gap: 8, padding: '8px 0' }}>
        ...
        const bslider = (lbl: string, key: string, hint: string) => {
          ...
          return (
            <div key={key} style={{ display: 'grid', gridTemplateColumns: '58px 1fr 30px', gap: 8, alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 11, color: '#9aa4b2' }}>{val}</span>
            </div>
          );
        };
        return (
      ...
      {variant === 'klein' && (
        <div>
          <label style={label}>Base preview — how many views to render</label>
          {segRow([{ v: 'front', label: 'Front only (fast)' }, { v: 'set', label: '4-view set' }],
                  baseSet ? 'set' : 'front', (v) => setBaseSet(v === 'set'))}
        </div>
      )}
      {variant === 'klein' && (
        <div>
  ...
  const poseSection = (generateLabel: string, onGenerate: () => void, canRun: boolean, required: boolean) => (
    <div style={sectionBox}>
          ...
          return (
            <div key={p.index}
              ...
              {p.thumb
                ...
                : <div style={{ height: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#9aa4b2' }}>{p.name}</div>}
      ...
      <button style={{ ...btn, marginTop: 10, opacity: canRun ? 1 : 0.5, cursor: canRun ? 'pointer' : 'not-allowed' }}
          ...
          <div style={{ display: 'grid', gap: 6 }}>
            {libRepos.map((r) => (
              <div key={r.repo_id} style={{ display: 'flex', alignItems: 'center', gap: 8,
                ...
                <span style={{ fontSize: 11, color: '#6b7280' }}>{r.pose_count || 0} poses</span>
                <div style={{ flex: 1 }} />
                <button style={{ ...btnGhost, padding: '3px 10px', fontSize: 11 }}
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: 24, color: '#e6e9ee' }}>
        ...
        <div style={{ ...box, marginBottom: 16 }}>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8 }}>
              ...
              <div><label style={label}>PuLID strength (def 1.4)</label>
                <input style={input} type="number" step="0.05" min="0" max="2" value={kpStrength}
      ...
      {tab === 'poselib' ? (
        ...
        <div style={box}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <h3 style={{ margin: 0, fontSize: 15 }}>VNCCS character library</h3>
            <div style={{ flex: 1 }} />
            <label style={{ fontSize: 12, color: '#9aa4b2' }}>Link to project:</label>
            <select style={{ ...input, width: 220 }} value={linkProject} onChange={(e) => setLinkProject(e.target.value)}>
              <option value="">(select project)</option>
              {projects.map((pr) => <option key={pr.id} value={pr.id}>{pr.name}</option>)}
            </select>
            <button style={btnGhost} onClick={loadLibrary}>Refresh</button>
          </div>
          {libMsg && <p style={{ fontSize: 13, color: '#5ee08a' }}>{libMsg}</p>}
          {!catalog.length && <p style={{ color: '#6b7280', fontSize: 13 }}>No cataloged characters yet — create (or just Save) one in the Create tab.</p>}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 10 }}>
            {catalog.map((c) => (
              <div key={c.character_id} style={{ background: '#0e1116', border: '1px solid #2a2f3a', borderRadius: 8, padding: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                  {c.hero_url && (
                    <img src={c.hero_url} alt={c.name}
                         style={{ width: 34, height: 34, objectFit: 'cover', borderRadius: 6,
                                  border: '1px solid #2a2f3a', background: '#0e1116' }} />
                  )}
                  <div style={{ fontWeight: 600 }}>{c.name}</div>
                  <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 6,
                                 background: c.variant === 'klein' ? '#1d2a3a' : '#241d3a',
                                 color: c.variant === 'klein' ? '#7fb8ff' : '#c9a8ff',
                                 border: '1px solid #2a2f3a' }}>
                    {c.variant === 'klein' ? '🧪 Klein' : '✨ Native'}
                  </span>
                </div>
                <div style={{ fontSize: 11, color: '#9aa4b2', marginBottom: 8 }}>
                  {Object.entries(c.outputs).map(([k, n]) => `${k}: ${n}`).join(' · ') || (c.form ? 'saved form (not generated yet)' : 'no outputs')}
                </div>
                <div style={{ display: 'grid', gap: 6 }}>
                  {c.form && (
                    <button style={{ ...btnGhost, width: '100%', padding: '6px 10px', fontSize: 12 }}
                            onClick={() => {
                              const v = c.variant === 'klein' ? 'klein' : 'native';
                              if (v !== variant) {
                                navigate(`/studio/${v === 'klein' ? 'vnccs-klein' : 'vnccs'}?char=${encodeURIComponent(c.name)}`);
                              } else loadIntoCreate(c);
                            }}>Load into Create</button>
                  )}
                  <button style={{ ...btn, width: '100%', padding: '6px 10px', fontSize: 12 }}
                          onClick={() => doLink(c.character_id)} disabled={!linkProject}>Link to project</button>
                  <button style={{ ...btnGhost, width: '100%', padding: '6px 10px', fontSize: 12, color: '#ff8a8a', borderColor: '#4a2a2a' }}
                          onClick={() => doDeleteCatalog(c)}>🗑 Delete character</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button style={{ ...btn, flex: 1, opacity: canPreview ? 1 : 0.5, cursor: canPreview ? 'pointer' : 'not-allowed' }}
                    ...
                    {previewBusy ? 'Rendering preview…' : '✨ Generate Character'}
                  </button>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                      ...
                      {enhanceOn && (
                        <div style={{ ...wizBox, display: 'grid', gap: 8, marginTop: 6 }}>
                          ...
                          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            {!enhanceBusy && enhanceMsg && <span style={{ fontSize: 11, color: '#9aa4b2' }}>{enhanceMsg}</span>}
                          </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button style={{ ...btn, flex: 1, opacity: canClonePreview ? 1 : 0.5, cursor: canClonePreview ? 'pointer' : 'not-allowed' }}
                    ...
                    {previewBusy ? 'Rendering preview…' : '✨ Generate Preview'}
                  </button>
        ...
        <div style={box}>
          ...
          {previewBusy && <p style={{ color: '#9aa4b2', fontSize: 13 }}>Rendering preview{host?.host ? ` on ${shortHost(host.host)}` : ''}… {previewElapsed}s elapsed</p>}
          {costPrevBusy && <p style={{ color: '#9aa4b2', fontSize: 13 }}>Dressing the mannequin on the host…</p>}
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                {groups.map(({ o, ims }) => (
                  <div key={o.label} style={{ marginBottom: 8 }}>
                    <div style={{ fontSize: 11, color: '#9aa4b2', margin: '6px 0 4px' }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              {baseVersions.length > 0 && host?.online && (
                <div style={{ marginTop: 8 }}>
                  ...
                  {baseEnhanceOn && (
                    <div style={{ ...wizBox, display: 'grid', gap: 8, marginTop: 6 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        {baseEnhanceBusy && (
                          <span style={{ fontSize: 11, color: '#9aa4b2', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#cbd2dc', marginTop: 10 }}>
          ...
          {chunks.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              {(() => {
                const done = chunks.filter((c) => c.status === 'done' || c.status === 'error').length;
                const pct = Math.round((done / chunks.length) * 100);
                return (
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#9aa4b2', marginBottom: 3 }}>
                      <span>{busy ? 'Generating…' : 'Run finished'}</span>
                      <span>{done}/{chunks.length} chunks · {pct}%</span>
                    </div>
                    <div style={{ height: 8, background: '#0e1116', border: '1px solid #2a2f3a', borderRadius: 6, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${pct}%`, background: pct === 100 ? '#166534' : '#3b82f6',
              ...
              <div style={{ display: 'grid', gap: 4, marginTop: 8 }}>
                {chunks.map((c, i) => (
                  <div key={`${c.prompt_id}-${i}`} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11,
                       background: '#0e1116', border: '1px solid #2a2f3a', borderRadius: 6, padding: '5px 9px' }}>
                    <span style={{
                      width: 8, height: 8, borderRadius: 8, flexShrink: 0,
                      background: c.status === 'done' ? '#5ee08a' : c.status === 'error' ? '#ff8a8a'
                        : c.status === 'ingesting' ? '#eab308' : '#3b82f6',
                    }} />
                    <span style={{ color: '#cbd2dc', fontWeight: 600 }}>{shortHost(c.host)}</span>
                    <span style={{ color: '#6b7280' }}>{c.label}</span>
                    <div style={{ flex: 1 }} />
                    <span style={{ color: '#9aa4b2' }}>
                      {c.status === 'running' ? (c.images.length ? `running · ${c.images.length} imgs` : 'running')
                        : c.status === 'ingesting' ? 'filing into library…'
                        : c.status === 'done' ? (c.images.length ? `done · ${c.images.length} imgs` : `done · ${c.note || 'filed'}`)
          ...
          {tab === 'create' && existingOutputs.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                ...
                {baseVersions.length > 0 && (
                  <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#9aa4b2' }}>
              ...
              {(showAllLibOutputs ? existingOutputs
                ...
                .map(({ o, cost, ims }) => {
                const imgs = (showAllVersions || !baseVersions.length || !curVersion)
                  ? ims
          ...
          {chunks.some((c) => c.images.length > 0) && (
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#9aa4b2', marginBottom: 6 }}>
